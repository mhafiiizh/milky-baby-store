        ...
        <div class="row">
            <div class="col-lg-9">
                ...
                <div class="row">
                    <?php
                // Query untuk halaman
                $queryCount = "SELECT COUNT(*) AS total FROM table_barang"; //Menghitung jumlah data lalu di aliaskan menjadi total
                $resultCount = mysqli_query($koneksi, $queryCount);
                $rowCount = mysqli_fetch_assoc($resultCount);
                $totalData = $rowCount['total']; // Memasukkan hasil jumlah data ke variable total data

                // Jumlah data yang ditampilkan per halaman
                $dataPerPage = 3;

                // Mendapatkan jumlah total halaman
                $totalPages = ceil($totalData / $dataPerPage); // ceil berfungsi untuk membulatkan ke atas

                // Mendapatkan halaman saat ini (jika tidak ada, set ke halaman pertama)
                $currentPage = isset($_GET['page']) ? $_GET['page'] : 1; // Operator tenary condition ? true : false
                        ...
                        <div class="card mb-4 product-wap rounded-0">
                            <form
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="mb-1">
                                        <?php echo "Rp". number_format($product_array[$key]["harga_barang"],0) ?></p>
